...
class BleManager:
    ...
    async def _scan(self, seconds: float) -> list[dict[str, Any]]:
        ...
        def _on_detect(device: BLEDevice, adv: AdvertisementData) -> None:
            ...
            # Update name if we find one. On Windows, device.name can sometimes
            # be the MAC address if the name isn't known yet; prefer a real name.
            ...
            if adv.service_uuids:
                ...
                for u in adv.service_uuids:
                    u_low = u.lower()
                    if u_low not in seen:
                        entry["service_uuids"].append(u_low)
                        seen.add(u_low)
